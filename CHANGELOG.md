...
### Breaking Changes
- Allow changing the order of the social network links that appear in the footer (#1152)
- `google-scholar` social network link no longer requires the prefix `citations?user=`; if you previously set this parameter, it needs to be updated (#1189)
...
This version has been in the works for a few years. It includes several new features that were highly requested, numerous bug fixes, new documentation, and aggressively encourages migrating from the old Google Universal Analytics to the new Analytics 4.
...
- Fixed page titles, subtitles, and excerpts rendering correctly when there are special characters in them (#856)
...
- All images (social media share, thumbnail, cover) can use either relative or absolute paths
...
- Use Twitter's large summary card (large image) when there is a cover image, thumbnail image, or share image specified (#668)
...
- Beautiful-Jekyll now supports the `remote_theme` config (#339) (thanks @gpotter2 and @skalee)
